# OpenTofu Runner Image

This repo contains the Dockerfile for building the default Spacelift OpenTofu runner image.

## Docker Repository

The image is pushed to the `public.ecr.aws/spacelift/runner-opentofu` public repository. It
is also pushed to the `ghcr.io/spacelift-io/runner-opentofu` as a backup in case of issues
with ECR.
...
- `spacelift-io/runner-opentofu:latest` -> with `aws` CLI
- `spacelift-io/runner-opentofu:gcp-latest` -> with `gcloud` CLI
- `spacelift-io/runner-opentofu:azure-latest` -> with `az` CLI
...
- `spacelift-io/runner-opentofu:latest-fips` -> with `aws` CLI & FIPS OpenSSL
- `spacelift-io/runner-opentofu:gcp-latest-fips` -> with `gcloud` CLI & FIPS OpenSSL
- `spacelift-io/runner-opentofu:azure-latest-fips` -> with `az` CLI & FIPS OpenSSL
